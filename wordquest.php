...
if (!isset($wqfunctions[$funcname]) || !is_callable($wqfunctions[$funcname])) {
	$wqfunctions[$funcname] = function() {
		...
		if ($dismiss) {
			...
			echo "<script>parent.document.getElementById('".$pluginslug."-reminder-notice').style.display = 'none';</script>";
		}
...
if (!isset($wqfunctions[$funcname]) || !is_callable($wqfunctions[$funcname])) {
	$wqfunctions[$funcname] = function() {
		...
		if ($wqdebug) {echo "<!-- Plugin Info: "; print_r($plugininfo); echo " -->";}
		return $plugininfo;
	};
}
		...
		foreach ($wordquestplugins as $pluginslug => $plugin) {
			if ($pluginslug != 'bioship') { // filter out theme here
				...
				if (isset($wordquestplugins[$pluginslug]['hasplans']) && $wordquestplugins[$pluginslug]['hasplans']) {
					if ($plugin['plan'] != 'premium') {echo "<option style='font-weight:bold;' value='upgrade'>".wqhelper_translate('Go PRO!')."</option>";}
